...
def train_epoch(current_epoch, seg_loss, model, optimizer, scheduler, train_data_loader,run):
    ...
    print("epoch: {}; lr {:.7f}; Loss {loss.avg:.4f}; Dice {dice.avg:.4f}".format(
                current_epoch, scheduler.get_lr()[-1], loss=losses, dice=dices))
    #wandb
    run.log({"epoch": current_epoch, "lr": scheduler.get_lr()[-1], "Loss": losses.avg, "Dice": dices.avg})
    return results
...
if __name__ == '__main__':
    ...
    for seg_loss in seg_loss_group:
        ...
        #init wandb
        run = wandb.init(project="ECEN649", name="res50_{}_loc".format(seg_loss),entity="tamucs",reinit=True,config={"batch_size": batch_size, "val_batch_size": val_batch_size,"seg_loss": seg_loss})
        ...
        np.save('result_res50.npy', results)
    #wandb finish
    run.finish()
