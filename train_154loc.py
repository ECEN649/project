...
def train_epoch(current_epoch, seg_loss, model, optimizer, scheduler, train_data_loader,run):
    losses = AverageMeter()
    ...
    for i, sample in enumerate(iterator):
        ...
        #wandb log
        run.log({"iter_lr": scheduler.get_lr()[-1], "iter_loss": losses.val, "iter_dice": dices.val})
        np.append(results, [current_epoch, losses, dices])
        #results.append([current_epoch, losses, dices])
    ...
    print("epoch: {}; lr {:.7f}; Loss {loss.avg:.4f}; Dice {dice.avg:.4f}".format(
                current_epoch, scheduler.get_lr()[-1], loss=losses, dice=dices))
    #wandb log
    run.log({"epoch": current_epoch, "lr": scheduler.get_lr()[-1], "loss": losses.val, "dice": dices.val})
...
if __name__ == '__main__':
    ...
    #每个seg_loss都使用一次
    for seg_loss in seg_loss_group:
        ...
        snapshot_name = snapshot_name + '_{}'.format(seg_loss)
        ...
        #init wandb
        run = wandb.init(project="ECEN649", name="se154_{}_loc".format(seg_loss),entity="tamucs",reinit=True,config={"batch_size": batch_size, "val_batch_size": val_batch_size,"seg_loss": seg_loss})
        ...
        seg_loss = ComboLoss(seg_loss, per_image=False).cuda()
        ...
        for epoch in range(30):
            result = train_epoch(epoch, seg_loss, model, optimizer, scheduler, train_data_loader,run)
            torch.cuda.empty_cache()
            ...
            #wandb log
            run.summary["Time"]=elapsed / 60
            run.summary["best_score"]=best_score
            run.summary["seg_loss"]=seg_loss
            run.summary["snapshot_name"]=snapshot_name
            run.summary["seed"]=seed
            run.summary["batch_size"]=batch_size
            run.summary["val_batch_size"]=val_batch_size
            run.summary["steps_per_epoch"]=steps_per_epoch
            run.summary["validation_steps"]=validation_steps


            results = np.array(results)
            #save results
            np.save('results_{}.npy'.format(snapshot_name), results)
        #wandb finish
        run.finish()